...
def monitor_loss(i, dataloader_len, step=0, tumor_shape_loss=0, tumor_grade_loss=0, inp_gb_loss=0, inp_lc_loss=0, \
    ...
    if step == 0:
        ...
        print(
            "\t[%d/%d]\t Inpaint loss G [gb, lc, grad, adv, content]: [%.4f, %.4f, %.4f, %.4f, %.4f] \t Inpaint loss D: %.4f " % (
                i, dataloader_len, inp_gb_loss, inp_lc_loss, inp_grad_loss, inp_adv_loss, content_loss, inp_dis_loss))
    else:
        print(
            "\t[%d/%d]\t Shape loss: %.4f \t Grade loss: %.4f \t Inpaint loss G [gb, lc, grad, adv, content]: [%.4f, %.4f, %.4f, %.4f, %.4f] \t Inpaint loss D: %.4f " % (
                i, dataloader_len, tumor_shape_loss, tumor_grade_loss, inp_gb_loss, inp_lc_loss, inp_grad_loss,
                inp_adv_loss, content_loss, inp_dis_loss))

    return 0


def save_result(result_dir, epoch, i, step, M=0, Binary_M=0, Circle_M=0, level_Circle_M=0, out_mask_shape=0,
                out_mask_grade=0, brain=0, uni_B=0, out_brain=0, mode='Train'):
    if mode == 'Train':
        if step == 0 or step == 2:
            result_img = torch.cat(
                (uni_B, Circle_M, out_mask_shape, Binary_M, Binary_M, level_Circle_M, out_mask_grade, M), 0)
    ...
    if mode == 'Test':
        if step == 0 or step == 2:
            ...
            error_F, error_T1, error_T1c, error_T2 = torch.split(torch.abs(brain - out_brain), split_size_or_sections=1,
                                                                 dim=1)
            result_img = torch.cat(
                (uni_B, Circle_M, coarse_mask, refine_mask, F, T1, T1c, T2, out_F, out_T1, out_T1c, out_T2, \
                 error_F, error_T1, error_T1c, error_T2), 0)
            vutils.save_image(result_img, filename=result_dir + "/[%03d]B_%04d.png" % (epoch, i), nrow=4)
