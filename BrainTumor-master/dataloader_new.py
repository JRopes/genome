...
class BrainDataset(Dataset):
    def __init__(self, root, mode, aug=False):
        if mode == 'Test':
            self.F = sorted(
                glob.glob(os.path.join(root, 'flair') + '/*.png'))
            self.T1 = sorted(
                glob.glob(os.path.join(root, 't1') + '/*.png'))
            self.T1c = sorted(
                glob.glob(os.path.join(root, 't1ce') + '/*.png'))
            self.T2 = sorted(
                glob.glob(os.path.join(root, 't2') + '/*.png'))
            self.M = sorted(
                glob.glob(os.path.join(root, 'seg') + '/*.png'))
            self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        return {'F': self.to_tensor(F), 'T1': self.to_tensor(T1), 'T1c': self.to_tensor(T1c), 'T2': self.to_tensor(T2),
                ...
                'level_Circle_M': level_Circle_M, 'uni_Circle_M': uni_Circle_M, 'instance_names': [self.F[index], self.T1[index], self.T1c[index], self.T2[index], self.M[index]]
                }
